...
@click.option(
    ...
    help="Optionally uou can choose to only run the deployment "
    "pipeline to train and deploy a model (`deploy`), or to "
    "only run the inference pipeline to make predictions on "
    "new data (`predict`), or to run both pipelines "
    "(`deploy_and_predict`)",   
)
...
def run_deployment(config: str, min_accuracy: float):
    ...
    if deploy:
        continuous_deployment_pipeline(
            data_path="/Users/prajwalamin/Documents/Development/MLOps/MLOps-fcc/data/olist_customers_dataset.csv",
